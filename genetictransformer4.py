...
class Transformer_Model(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        tgt_emb = self.pos_encoder(tgt_emb.to(device))
        logging.debug(f"tgt_emb Shape: {tgt_emb.shape}")

        if self.training:
            # In training mode, use fixed seq_len
            tgt_mask = nn.Transformer.generate_square_subsequent_mask(self.seq_length).to(device)
            #tgt_mask = pad_mask(tgt_mask, seq_len)  # Pad to match the full seq_len

        else:
            # In inference mode, dynamically adjust to the current length of decoder_input
            current_length = tgt_emb.size(1)
            tgt_mask = nn.Transformer.generate_square_subsequent_mask(current_length).to(device)


        output = self.transformer(src_emb.to(device), tgt_emb.to(device), tgt_mask=tgt_mask.to(device)).to(device)
...
def propagate_embedding_size(new_model, new_dim):
    ...
    if hasattr(new_model, 'pos_encoder'):
        ...
        print(f"Positional encoding updated for new embedding size: {new_dim}")
    ...
    if hasattr(new_model, 'transformer') and isinstance(new_model.transformer, nn.Transformer):
        ...
        print(f"Transformer layers updated for new embedding size: {new_dim}")
    ...
    if hasattr(new_model, 'fc_out'):
        ...
        print(f"Output projection layer updated for new embedding size: {new_dim}")
    ...
    if hasattr(new_model.transformer.encoder, 'layers'):
        for layer in new_model.transformer.encoder.layers:
            if hasattr(layer, 'self_attn') and isinstance(layer.self_attn, nn.MultiheadAttention):
                ...
                print("MultiheadAttention layers updated.")
    if hasattr(new_model.transformer.decoder, 'layers'):
        for layer in new_model.transformer.decoder.layers:
            if hasattr(layer, 'self_attn') and isinstance(layer.self_attn, nn.MultiheadAttention):
                ...
                layer.self_attn.k_proj_weight = nn.Parameter(torch.randn(new_dim, new_dim))
                layer.self_attn.v_proj_weight = nn.Parameter(torch.randn(new_dim, new_dim))
                print("Decoder MultiheadAttention layers updated.")
...
class FireflyOptimizer:
    ...
    def calculate_fitness(self, model, loss_fn, inputs, targets, architecture):
...
def check_layer_registration(model):
    print("\n[DEBUG] Checking Layer Registration...")
    for name, param in model.named_parameters():
        print(f"{name}: {param.size()} - Requires Grad: {param.requires_grad}")
    print("[DEBUG] Layer Registration Check Complete.\n")
...
def propagate_population_size(new_model, population):
    ...
    for model in population:
        if hasattr(model, 'embedding') and hasattr(new_model, 'embedding'):
            ...
            print(f"Population model embedding updated to size: {new_dim}")
...
class HybridNEATFireflyOptimizer:
    def __init__(self, model, loss_fn, population_size=5, mutation_rate=0.1, crossover_rate=0.1, embed_growth_rate=0.001, alpha=0.1, beta=0.5, learning_rate=0.001):
        ...
        self.optimizers = [torch.optim.Adam(m.parameters(), lr=self.learning_rate) for m in self.population]
        self.fitness_history = []

    def calculate_fitness(self, loss_fn, model, inputs, targets, architecture):
        total_loss = 0
        check_layer_registration(model)
    ...
    def optimize(self, loss_fn, inputs, targets, architecture):
        ...
        fitness_scores = [self.calculate_fitness(loss_fn, m, inputs, targets, architecture) for m in self.population]
...
class ReasoningModelGUI:
            ...
            try:
                if self.architecture.get() == "Reasoning Model LNS":
                    ...
                    with torch.no_grad():
                        ...
                        input_ids = torch.cat([input_ids, torch.full((1, seq_len - input_ids.size(1)), pad_token_id, dtype=torch.long, device=device)], dim=1)

                        logging.debug(f"Padded input_ids Shape: {input_ids.shape}")

                        # Choose a start token for the dummy target.
                        bos_token = self.tokenizer.bos_token_id if self.tokenizer.bos_token_id is not None else pad_token_id
                        bos_token  = torch.tensor([[bos_token]], device=device)
                        tgt_ids = torch.cat([bos_token, torch.full((1, seq_len - input_ids.size(1)), pad_token_id, dtype=torch.long, device=device)], dim=1)

                        # Keep track of the original input length
                        input_length = input_ids.size(1)

                        for _ in range(seq_len):  # Loop until the full sequence length
                            # Check the target mask generation
                            tgt_mask = nn.Transformer.generate_square_subsequent_mask(tgt_ids.size(1)).to(device)

                            tgt_mask = pad_mask(tgt_mask, seq_len)
                            
                            outputs = self.model(input_ids, tgt_ids)
                            next_token_logits = outputs[:, tgt_ids.size(1) - 1, :]
                            ...
                            if next_token_id.item() == self.tokenizer.eos_token_id or tgt_ids.size(1) >= 10:
                                break
                        generated_text = self.tokenizer.decode(tgt_ids[0].tolist(), skip_special_tokens=False)
    ...
    def training_loop(self):
        ...
        try:
            ...
            loss_fn = nn.CrossEntropyLoss(label_smoothing=0.1, ignore_index=pad_token_id)
            ...
            for epoch in range(self.epochs.get()):
                ...
                for batch_idx, (batch_input_ids, batch_labels) in enumerate(dataloader):
                    if self.stop_training.is_set():
                            ...
                            messagebox.showinfo("Info", "Training stopped by user.")
                            return
                    check_layer_registration(self.model)
                    ...
                    if self.use_genetic_algo == "Genetic Algorithm":
                            ...
                            self.model = firefly_optimizer.optimize(loss_fn, batch_input_ids, target_labels, architecture)
                        ...
                        self.model = hybrid_optimizer.optimize(loss_fn, batch_input_ids, target_labels, architecture)
                        ...
                        if architecture == "Reasoning Model LNS":
                            ...
                            loss = log_space_cross_entropy(output, target_labels)
                        else:
