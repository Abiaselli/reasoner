...
class Muon(Optimizer):
    def __init__(self, params, lr=1e-3, momentum=0.9, weight_decay=0.0, 
                 ns_iterations=5, a=3.4445, b=-4.7750, c=2.0315):
        # Default hyperparameters based on Muon paper recommendations
        defaults = dict(lr=lr, momentum=momentum, weight_decay=weight_decay,
                        ns_iterations=ns_iterations, a=a, b=b, c=c)
        super().__init__(params, defaults)
    
    def step(self, closure=None):
        if closure is not None:
            closure()  # Allows loss re-evaluation if provided
        for group in self.param_groups:
            lr = group['lr']; mu = group['momentum']; wd = group['weight_decay']
            a = group['a']; b = group['b']; c = group['c']; N = group['ns_iterations']
            for p in group['params']:
                ...
                # Momentum update: M_t = mu * M_(t-1) + grad_t
                M = state['momentum_buffer']
                M.mul_(mu).add_(grad)  # in-place update of momentum buffer

                # Prepare M as a matrix for orthogonalization
                if M.dim() <= 1:
                    # For 1D or scalar, treat as 1 x N matrix
                    M_mat = M.view(1, -1)
                else:
                    # Flatten all but the first dimension into the second dimension
                    M_mat = M.view(M.shape[0], -1)
                
                # Newton-Schulz iteration to orthogonalize M_mat
                # X0 = M / frobenius_norm(M)
                frob_norm = M_mat.norm(p='fro')
                if frob_norm == 0 or torch.isnan(frob_norm):
                    O_mat = torch.zeros_like(M_mat)  # zero or invalid gradient -> no update
                else:
                    X = M_mat / frob_norm
                    for k in range(N):
                        # Compute X * X^T and its square
                        X_XT = X.mm(X.t())               # matrix of shape [r, r]
                        X_XT2 = X_XT.mm(X_XT)            # squared matrix [r, r]
                        # Update X: a*X + b*(X_XT * X) + c*(X_XT2 * X)
                        X = a * X + b * X_XT.mm(X) + c * X_XT2.mm(X)
                    O_mat = X  # result of orthogonalization
                
                # Reshape O_mat back to original parameter shape
                O = O_mat.view_as(p.data)
                
                # Apply weight decay (decoupled): W <- W * (1 - lr*wd)
                if wd != 0:
                    p.data.mul_(1 - lr * wd)
                # Apply the orthogonalized update: W <- W - lr * O
                p.data.add_(-lr, O)
        return None
...
class ReasoningModelGUI:
    ...
    def training_loop(self):
        ...
        try:
            ...
            optimizer = Muon(self.model.parameters(), lr=lr, momentum=0.9, weight_decay=1e-5)
