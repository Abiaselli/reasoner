        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if M.dim() <= 1:
                    ...
                    M_mat = M.view(1, -1).contiguous()
                else:
                    # Flatten all but the first dimension into the second dimension
                    M_mat = M.view(M.shape[0], -1).contiguous()
                ...
                if frob_norm == 0 or torch.isnan(frob_norm):
                    ...
                    for k in range(N):
                        ...
                        X_XT = X.mm(X.t().contiguous()).contiguous()               # matrix of shape [r, r]
                        X_XT2 = X_XT.mm(X_XT.contiguous()).contiguous()            # squared matrix [r, r]
                        # Update X: a*X + b*(X_XT * X) + c*(X_XT2 * X)
                        X = a.contiguous() * X.contiguous() + b.contiguous() * X_XT.mm(X) + c * X_XT2.mm(X)
