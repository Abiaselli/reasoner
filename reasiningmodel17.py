...
def collate_fn(batch):
    if len(batch[0]) == 3:
        ...
        return input_ids, labels, labels_tot, seq_lengths
    else:
...
class Transformer_Model(nn.Module):
    ...
    def forward(self, input_ids, targets, reason_targets, mask=None):
        ...
        attn_output = self.attention(thought, mask)
        logging.debug(f"Attention Output Shape: {attn_output.shape}")  # Log attention output

        confidence = False
        attempt = 0
        max_attempts = attempt_limit

        thought = self.output_projection(attn_output).to(dtype=torch.float32)
        logging.debug(f"Output Projection Shape1: {thought.shape}")  # Log output shape
        ...
        logging.debug(f"Gumnel thought Shape model: {thought.shape}") 


        while not confidence and attempt < max_attempts:
            thought = self.examination[attempt % len(self.examination)](thought.to(dtype=torch.float32), reason_targets)
...
class Examination(nn.Module):
    ...
    def forward(self, x, exam_ids):
        ...
        loss = self.loss_fn(test_thought.view(-1, self.vocab_size), exam_ids.view(-1))
        logging.debug(f"Exam loss: {loss.item()}")  
...
class Imagination(nn.Module):
    ...
    def forward(self, x, input_ids):
        logging.debug(f"Imagine thought Shape 1: {x.shape}")  # Log output shape
        emb_weight = self.embedding.embedding.weight.clone()

        thought = torch.matmul(x, emb_weight)         
        logging.debug(f"Imagine thought Shape 1: {thought.shape}")  # Log output shape
...
class Reasoning(nn.Module):
    ...
    def forward(self, x, targets):
        logging.debug(f"Reasoning input Shape: {x.shape}") 
        emb_weight = self.embedding.embedding.weight.clone()

        last_thought = torch.matmul(x, emb_weight) 
        logging.debug(f"Reasoning last_thought Shape: {last_thought.shape}")  
        ...
        loss = self.loss_fn(test_thought.view(-1, self.vocab_size), targets.view(-1))
        confidence = False

        if loss < training_goal:
            confidence = True
        elif confidence == False:
            thought = self.layer(last_thought)
            logging.debug(f"Reasoning thought Shape 4: {thought.shape}") 
            thought = self.output_projection(thought).to(dtype=torch.float32)
            logging.debug(f"Reasoning thought Shape 5: {thought.shape}") 
            test_thought = thought.clone()
            logging.debug(f"Reasoning thought Shape 6: {test_thought.shape}") 
            loss = self.loss_fn(test_thought.view(-1, self.vocab_size), targets.view(-1))
        temperature = 1  # Lower temperature → more one-hot-like outputs
        thought = gumbel_softmax(thought.clone(), temperature=temperature, hard=True)
...
class Validate(nn.Module):
    ...
    def forward(self, x, targets):
        ...
        last_thought = torch.matmul(x, emb_weight) 
        logging.debug(f"Doubt thought Shape 1: {last_thought.shape}")  
        thought = self.layer(last_thought)
        thought = self.output_projection(thought).to(dtype=torch.float32)
        test_thought = thought.clone()
        loss = self.loss_fn(test_thought.view(-1, self.vocab_size), targets.view(-1))
...
class Reinforcement(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, seq_length, num_layers):
        ...
        self.layer = Transformer_Layer(vocab_size, hidden_dim)
        self.loss_fn = nn.CrossEntropyLoss()
        self.output_projection = nn.Linear(embedding_dim, vocab_size)
        self.reward_scale = 0.1  # Scaling factor for rewards
        self.vocab_size = vocab_size
        self.embedding = Transformer_Token_Embedding(vocab_size, embedding_dim)  # Embedding layer    

    def forward(self, x, targets, attempts):
        logging.debug(f"Reward input Shape: {x.shape}") 
        emb_weight = self.embedding.embedding.weight.clone()

        thought = torch.matmul(x, emb_weight) 
        logging.debug(f"Reward thought Shape 1: {thought.shape}") 
        thought = self.output_projection(thought)
        test_thought = thought.clone()
        logging.debug(f"Reward thought Shape 2: {test_thought.shape}") 
        loss = self.loss_fn(test_thought.view(-1, self.vocab_size), targets.view(-1))
        if loss < training_goal:
            reward = attempt_limit / (attempts + 1)  # Higher reward for fewer attempts
        else:
            reward = max(-0.01, -loss)  # Prevent extreme negative rewards
            logging.debug(f"Reward =: {reward}") 

            thought = self.layer(thought.clone())
            logging.debug(f"Reward thought Shape 3: {thought.shape}") 
            test_thought = thought.clone()
            logging.debug(f"Reward thought Shape 4: {test_thought.shape}") 
            loss = self.loss_fn(test_thought.view(-1, self.vocab_size), targets.view(-1))
            if loss < training_goal:
                return thought, reward
...
class QuaternionGeneticAlgorithm:
    ...
    def select_best(self, loss_fn, inputs, targets, tot_targets, mask):
        ...
        for model in self.population:
            ...
            loss1 = 0
            loss2 = 0
            logits, reward, train_of_thought_logits = model(inputs, targets, tot_targets, mask)
            ...
            if loss < best_loss:
                    best_loss = loss
                    n=n+1
                    print(f"Best model iteration {n}, Loss: {loss.item()}, reg: {loss1.item()}, tot: {loss2.item()}")
                    best_model = model
                ...
                loss = alpha * loss1 + beta * loss2
                n=n+1
                print(f"Iteration {n}, Loss: {loss}, regular: {loss1.item()}, tot: {loss2.item()}")
                if loss < best_loss:
                        best_loss = loss
                        n=n+1
                        print(f"Best model iteration {n}, Loss: {loss.item()}, regular: {loss1.item()}, tot: {loss2.item()}")
                        best_model = model
                logging.debug(f"Logits_flat shape: {logits_flat.shape}")
                logging.debug(f"targets_flat shape: {targets.shape}")    
        return best_model
    ...
    def training_loop(self):
        ...
        try:
            ...
            for epoch in range(self.epochs.get()):
                ...
                # Training loop
                for batch_idx, (batch_input_ids, batch_labels, batch_labels_tot, seq_lengths) in enumerate(dataloader):
                    if self.stop_training.is_set():
                        logging.info("Training stopped by user.")
                        messagebox.showinfo("Info", "Training stopped by user.")
                        return
                    ...
                    if self.use_genetic_algo == "Genetic Algorithm":
                        ...
                        self.model = qga.evolve(loss_fn, batch_input_ids, targets_reshaped, tot_targets_reshaped, mask=None)
                    ...
                    # Forward pass
                        try:
                            ...
                            outputs, reward, train_of_thought_logits = self.model(batch_input_ids, targets_reshaped, tot_targets_reshaped)
                        ...
                        loss1 = loss_fn(logits_reshaped, targets_reshaped)
                        loss2 = loss_fn(tot_logits_flat, tot_targets_reshaped)
                        ...
                        logging.info(f"Loss computed: {loss.item()}, reg: {loss1.item()}, tot: {loss2.item()}")
                        ...
                        n+=1
                        print(f"Iteration {n}, Loss: {loss.item()},reg: {loss1.item()}, tot: {loss2.item()}")
