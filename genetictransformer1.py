...
class Transformer_Model(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        # for autoregressive gpt style decoding add this to decoded_output
...
class HybridNEATFireflyOptimizer:
    ...
    def crossover(self, parent1, parent2, fitness1, fitness2):
        logging.debug('crossover attempt')
        child = copy.deepcopy(parent1)
        # Weighted crossover favoring higher fitness parents
        crossover_weight = fitness1 / (fitness1 + fitness2 + 1e-8)
        
        for child_param, param1, param2 in zip(child.parameters(), parent1.parameters(), parent2.parameters()):
            crossover_mask = (torch.rand_like(child_param) < crossover_weight).float()
            child_param.data = param1.data * crossover_mask + param2.data * (1 - crossover_mask)
        logging.debug('crossover complete')
        return child
    ...
    def optimize(self, loss_fn, inputs, targets, architecture):
        ...
        # Identify the brightest (best) model
        best_idx = torch.argmax(torch.tensor(fitness_scores))
        best_model = self.population[best_idx]

        # Firefly Movement: Attract less fit models towards more fit ones
        for i in range(len(self.population)):
            if i != best_idx:
                self.move_towards(self.population[i], best_model, fitness_scores[i], fitness_scores[best_idx])
        
        # Crossover and Mutation for next generation
        next_population = []
        for _ in range(len(self.population) // 2):
            parent1, parent2 = random.sample(self.population, 2)
            fitness1, fitness2 = fitness_scores[self.population.index(parent1)], fitness_scores[self.population.index(parent2)]
            child = self.crossover(parent1, parent2, fitness1, fitness2)
            next_population.append(child)

        # Mutate the best model's copies
        for _ in range(len(self.population) - len(next_population)):
            mutated_model = self.mutate_topology(copy.deepcopy(best_model), fitness_scores[best_idx])
            next_population.append(mutated_model)
        n=0
        # Backpropagation Update
        for model, optimizer in zip(next_population, self.optimizers):
                model.train()
                optimizer.zero_grad()
                if architecture == "Reasoning Model LNS":
                    decoder_input = targets[:, :-1]
            ...
            try:
                ...
                # print(output_text)
                    ...
                    with torch.no_grad():
                        ...
                        generated_text = self.tokenizer.decode(input_ids[0].tolist(), skip_special_tokens=False)
