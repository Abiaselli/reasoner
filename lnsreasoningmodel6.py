...
class Transformer_Model_LNS(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_layers, seq_length, num_heads):
        ...
        self.examination = ExaminationLNS(vocab_size, embedding_dim, hidden_dim) 
        self.reasoning = ReasoningLNS(vocab_size, embedding_dim, hidden_dim)
        self.doubt = ValidateLNS(vocab_size, embedding_dim, hidden_dim) 
        self.imagine = ImaginationLNS(vocab_size, embedding_dim, hidden_dim)
        self.reinforcement = ReinforcementLNS(vocab_size, embedding_dim, hidden_dim, num_layers)
        self.output_projection = LinearLNS(embedding_dim, vocab_size)
        self.pos_encoder = EmbeddingLNS(seq_length, embedding_dim)
    ...
    def forward(self, input_ids, mask):
        ...
        src = self.lns.log_add(thought, pos_encodings)

        # Forward through transformer encoder (self-attention)
        encode_thought = self.transformer_encoder(src, mask) #change to Log space
        ...
        decoded_output = self.decoder(log_tgt_input, encode_thought, tgt_mask=None, memory_mask=mask)
        logging.debug(f"Decoded Output Shape: {decoded_output.shape}")

        memory_mask = create_memory_mask(encode_thought)

        # Output projection to vocab size
        train_of_thought = self.output_projection(decoded_output.clone())
        ...
        thought = self.examination(decoded_output.to(dtype=torch.float32))
        logging.debug(f"Examination Output Shape: {thought.shape}")  # Log examination output

        thought, attention_weights = self.cross_node_attention(encode_thought, thought, thought, memory_mask) 
        logging.debug(f"Attention Output Shape: {thought.shape}")  # Log attention output
        ...
        final_output_logits = self.output_projection(thought)
        logging.debug(f"Final Output Logits Shape: {final_output_logits.shape}")

        # **Concatenate Train of Thought Logits and Final Output Logits**
        concatenated_logits = torch.cat((train_of_thought, final_output_logits), dim=1)
        logging.debug(f"Concatenated Logits Shape: {concatenated_logits.shape}")

        return concatenated_logits

### EXAMINATION MODULE ###
class ExaminationLNS(nn.Module):
    """
    Module to examine inputs to determine issue and path forward
    """
    def __init__(self, vocab_size, embedding_dim, hidden_dim):
        super(ExaminationLNS, self).__init__()
        self.layer = FeedForwardLNS(embedding_dim, hidden_dim)
        self.output_projection = LinearLNS(embedding_dim, vocab_size)
        self.vocab_size = vocab_size
    ...
    def forward(self, x):
        ...
        thought = self.lns.to_log(thought.clone()) 
        emb_weight = self.lns.to_log(self.embedding.embedding.weight.clone())

        thought = self.lns.mul(thought, emb_weight)         
...
class ImaginationLNS(nn.Module):
    """
    Module for thought mutation
    """
    def __init__(self, vocab_size, embedding_dim, hidden_dim):
        super(ImaginationLNS, self).__init__()
        self.embedding = Transformer_Token_Embedding(vocab_size, embedding_dim)  # Embedding layer    
        self.layer = FeedForwardLNS(embedding_dim, hidden_dim)
        self.output_projection = LinearLNS(embedding_dim, vocab_size)
        self.lns = LogarithmicNumberSystem()
...
### REASONING MODULE ###
class ReasoningLNS(nn.Module):
    """
    Module for train of thought reasoning
    """
    def __init__(self, vocab_size, embedding_dim, hidden_dim):
        super(ReasoningLNS, self).__init__()
        self.embedding = Transformer_Token_Embedding(vocab_size, embedding_dim)  # Embedding layer    
        self.layer = FeedForwardLNS(embedding_dim, hidden_dim)
        self.output_projection = LinearLNS(embedding_dim, vocab_size)
        self.vocab_size = vocab_size
...
### VALIDATION MODULE ###
class ValidateLNS(nn.Module):
    """
    Transformer module to second guess itself
    """
    def __init__(self, vocab_size, embedding_dim, hidden_dim):
        super(ValidateLNS, self).__init__()
        self.embedding = Transformer_Token_Embedding(vocab_size, embedding_dim)  # Embedding layer    
        self.layer = FeedForwardLNS(embedding_dim, hidden_dim)
        self.output_projection = LinearLNS(embedding_dim, vocab_size)
        self.vocab_size = vocab_size
...
class ReinforcementLNS(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_layers):
        super(ReinforcementLNS, self).__init__()
        self.layers = nn.ModuleList([FeedForwardLNS(embedding_dim, hidden_dim) for _ in range(num_layers)])
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, num_heads, dim_feedforward, dropout=0.1):
        ...
        self.self_attn = MultiheadAttentionLNS(d_model, num_heads, dropout=dropout)
        self.multihead_attn = MultiheadAttentionLNS(d_model, num_heads, dropout=dropout)
...
def create_memory_mask(memory, pad_token_id=9996):
    ...
    """
    return (memory != pad_token_id).unsqueeze(1)  # Shape: [batch_size, 1, seq_len]
...
class Transformer_Model(nn.Module):
    ...
    def forward(self, input_ids, mask):
        ...
        logging.debug(f"src Shape: {src.shape}")  # Log attention output

        # Forward through transformer encoder (self-attention)
        encode_thought = self.transformer_encoder(src, mask)

        # Prepare input for the decoder (e.g., <start> token or shifted input)
        tgt_input = torch.zeros_like(input_ids).fill_(self.vocab_size-1)  # <start> token
        tgt_pos_encodings = self.pos_encoder(torch.arange(seq_length, device=input_ids.device).unsqueeze(0))
        tgt_pos_encodings = tgt_pos_encodings.expand(batch_size, seq_length, -1)
        tgt = self.embedding(tgt_input) + tgt_pos_encodings
        logging.debug(f"tgt shape: {tgt.shape}")  # Log attention output

        # Decode
        decoded_output = self.decoder(tgt, encode_thought, tgt_mask=None, memory_mask=mask)
        
        attention_weights = None
        # Output projection to vocab size
        train_of_thought = self.output_projection(decoded_output.clone())
        logging.debug(f"Attention Output Shape: {decoded_output.shape}")  # Log attention output
...
class GeneticAlgorithm:
    ...
    def select_best(self, loss_fn, inputs, targets, tot_targets, mask):
        ...
        for model in self.population:
            loss = 0
            concatenated_logits = model(inputs, mask)
            # Flatten logits and targets:
            if torch.isnan(concatenated_logits).any() or torch.isinf(concatenated_logits).any():
            ...
            concatenated_labels = torch.cat((tot_targets, targets), dim=1)
            logging.debug(f"Concatenated Labels Shape: {concatenated_labels.shape}")

            loss += loss_fn(concatenated_logits.view(-1, concatenated_logits.size(-1)), concatenated_labels.view(-1))
    ...
    def test_inference(self):
        ...
        if sample_input:
            try:
                ...
                predicted_text = self.tokenizer.decode(input_ids[0], skip_special_tokens=True)
    ...
    def training_loop(self):
        ...
        try:
            if self.use_chunked_dataset.get():
                ...
                pad_token_id = self.tokenizer.pad_token_id if tokenizer else 9996  # Default to 9996 if tokenizer isn't set      
                ...
                for batch_idx, (batch_input_ids, batch_labels, batch_labels_tot, seq_lengths, batch_attention_masks) in enumerate(dataloader):
                    ...
                    while inner_updates < max_inner_updates and current_loss >= previous_loss:
                        ...
                        if self.use_genetic_algo == "Genetic Algorithm":
                            ...
                            self.model = qga.evolve(loss_fn, batch_input_ids, batch_labels, batch_labels_tot, mask=combined_mask)
                        ...
                        # Forward pass
                            try:
                                ...
                                concatenated_logits = self.model(batch_input_ids, combined_mask)
                                if torch.isnan(concatenated_logits).any() or torch.isinf(concatenated_logits).any():
                            ...
                            logging.debug(f"Shape of outputs: {concatenated_logits.shape}")
                            # Assume batch_labels and batch_labels_tot are tensors of shape [batch_size, seq_len, vocab_size]
                            concatenated_labels = torch.cat((batch_labels_tot, batch_labels), dim=1)
                            logging.debug(f"Concatenated Labels Shape: {concatenated_labels.shape}")


                            # Compute loss
                            loss_fn = nn.CrossEntropyLoss(ignore_index=0)  # Assuming 0 is the padding token
                            loss = loss_fn(concatenated_logits.view(-1, concatenated_logits.size(-1)), concatenated_labels.view(-1))
