...
class Transformer_Model(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads, seq_length, device=device):
        super().__init__()
    ...
    def forward(self, src, tgt_ids):
        src_pad_mask, tgt_pad_mask, causal_mask = self.generate_mask(src, tgt_ids)
...
class ReasoningModelGUI:
    def __init__(self, root):
        ...
        self.vocab_path = ""
        self.tokenizer_path = ""
        self.batch_size = tk.IntVar(value=18)
                            ...
                            while loss_diff >= 0 and (attempt % 4) != 0:
                                ...
                                try:
                                    if architecture == "Reasoning Model LNS":
                                        ...
                                        output = checkpoint.checkpoint(self.model, batch_input_ids, decoder_input,  use_reentrant=False)
                                except Exception as e:
                                ...
                                if architecture == "Reasoning Model LNS":
                                    ...
                                    logging.debug(f"loss debug LNS transformer: {loss}")
                                ...
                                loss.backward()
                                prev_loss = loss.item()
                                if prev_loss > 12:  # Or whatever threshold you consider "too high"
                                    print("\n--- ⚠️ High Loss Batch Detected ---")
                                    print(f"Loss: {loss.item():.4f}")
                                    
                                    # Show input and target token IDs
                                    print("Input IDs:\n", batch_input_ids)
                                    print("Target IDs (reshaped):\n", target_labels.view(batch_input_ids.shape))

                                    # Optional: decode to text for readability
                                    if tokenizer:
                                        try:
                                            for i in range(min(3, batch_input_ids.size(0))):  # limit to 3 examples
                                                print(f"\nSample {i+1}:")
                                                print("Input Text:", tokenizer.decode(batch_input_ids[i], skip_special_tokens=True))
                                                print("Target Text:", tokenizer.decode(target_labels[i][:batch_input_ids.shape[1]], skip_special_tokens=True))
                                        except Exception as e:
                                            print("⚠️ Token decoding failed:", e)

                                    print("--- End of High Loss Batch ---\n")
                                ...
                                new_loss = loss_fn(output[:, :seq_len, :].reshape(-1, output.shape[-1]), 
